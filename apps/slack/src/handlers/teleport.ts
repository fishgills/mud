...
export class TeleportHandler extends PlayerCommandHandler {
  constructor() {
    super(COMMANDS.TELEPORT, 'Teleport failed');
  }
  ...
  protected async perform({
    ...
    if (!result.success) {
      await say({ text: result.message ?? 'Teleport failed.' });
    ...
    if (result.state === 'awaiting_choice') {
      ...
      this.app.logger.info(
        ...
        'Teleport handler awaiting choice',
      );
      return;
    }
    ...
    if (destination) {
      ...
      try {
        ...
        this.app.logger.warn(
          { err, userId, destination },
          'Teleport occupants summary failed',
        );
      }
    ...
    this.app.logger.info(
      ...
      'Teleport handler exited HQ',
    );
  }
}

export const teleportHandler = new TeleportHandler();
