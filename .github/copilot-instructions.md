...
- **Backend**: NestJS services communicating via ts-rest REST APIs
...
- **ts-rest + Zod** - Type-safe REST API contracts shared across services
...
- **@ts-rest/core** - Client generation for shared API contracts
- **ESLint** - Code linting
...
- Expose routes through shared ts-rest contracts defined in `libs/api-contracts`
- Use interceptors for cross-cutting concerns (logging, caching)

### ts-rest Contracts

- Contracts live in `libs/api-contracts` and are the single source of truth for request/response shapes
- Update both the Zod schema and exported TypeScript types when changing an endpoint
- Consumers (Slack bot, tick worker, DM service) should import the shared contract and create clients with `initClient`
- Avoid hand-rolled fetch wrappers—reuse the existing clients where possible
...
- Return consistent REST error payloads with meaningful HTTP status codes
...
- Use shared ts-rest contracts for service-to-service calls
...
3. **Type Safety**: Leverage shared ts-rest contract types instead of `any`
...
10. **Service URLs**: Ensure service endpoints align with the shared contract base URLs
...
- Review API contracts in `libs/api-contracts`
